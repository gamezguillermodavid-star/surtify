...
- **Perfil de conductor profesional (opcional)**: el usuario puede marcarse como taxista, VTC, transportista o repartidor en el registro. Este perfil tiene sus propias misiones e insignias ("Kilómetros de confianza") y sirve además como segmentación real de cara a futuros acuerdos con gasolineras, ya que es el colectivo con mayor volumen de repostaje. *(Implementado parcialmente: no se pide en el registro, sino que se elige con un selector particular/profesional en el Perfil (`DriverTypeSelector`); no hay todavía misiones, insignias ni segmentación específica para el perfil profesional. Ver `CLAUDE.md` para el estado real.)*
